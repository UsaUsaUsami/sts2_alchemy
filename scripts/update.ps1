...
$base = Get-Content -LiteralPath (Join-Path $GameRoot 'mods/BaseLib/BaseLib.json') -Raw | ConvertFrom-Json
...
$manifest = Get-Content -LiteralPath (Join-Path $source 'Alchemist.json') -Raw | ConvertFrom-Json
$existing = Get-Content -LiteralPath (Join-Path $destination 'Alchemist.json') -Raw | ConvertFrom-Json
